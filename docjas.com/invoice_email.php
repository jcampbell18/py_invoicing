<?
    ...
    $sqlA = "SELECT l.name,l.address,l.city,l.state,l.zipcode,l.phone,l.email,c.contact_name,c.business_name,c.address,c.city,c.state,c.zipcode,c.phone,p.address,p.city,p.state,p.zipcode,s.name,t.name FROM login l, clients c, projectsites p, sku s, terms t WHERE p.id='$project_id' AND c.id='$client_id' AND s.id='$sku_id' AND t.id='$term_id'";
    $sqlA = mysql_query($sqlA);
	list($l_name,$l_address,$l_city,$l_state,$l_zipcode,$l_phone,$l_email,$c_contact_name,$c_business_name,$c_address,$c_city,$c_state,$c_zipcode,$c_phone,$p_address,$p_city,$p_state,$p_zipcode,$s_name,$t_name) = mysql_fetch_row($sqlA);
?>
...
<head>
  <title><?= 'INV#'.$inv.' '.$p_address.'_'.$p_city ?></title>
